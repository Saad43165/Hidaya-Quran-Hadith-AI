...
export function TasbihScreen() {
  ...
  useEffect(() => {
    AsyncStorage.getItem(STORAGE_KEY).then(v => {
      if (v) setTotalAllTime(parseInt(v, 10));
    });
  }, []);
  ...
  const handleCount = () => {
    ...
    AsyncStorage.setItem(STORAGE_KEY, String(newTotal));

    // Vibrate on completion of a round
    if (newCount % selected.target === 0) {
      Haptics.success();
    }
  };
    ...
    <ScreenContainer noPadding>
      ...
      >
        {ZIKR_OPTIONS.map((z, i) => (
          <TouchableOpacity
            ...
            onPress={() => { setSelectedIdx(i); setCount(0); }}
          >
      ...
      <View style={styles.counterArea}>
          ...
          <Animated.View style={{ transform: [{ scale: scaleAnim }] }}>
            ...
            >
              <LinearGradient
                ...
                style={styles.counterBtnGrad}
              >
                <Text style={[styles.countNumber, { color: selected.color }]}>{remainder || (laps > 0 ? selected.target : 0)}</Text>
                {laps > 0 && <Text style={styles.lapText}>{laps} × {selected.target}</Text>}
        ...
        <View style={styles.progressWrap}>
          ...
          <Text style={styles.progressText}>{remainder} / {selected.target}</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  ripple: {
    position: 'absolute', width: BTN_SIZE, height: BTN_SIZE,
    borderRadius: BTN_SIZE / 2, borderWidth: 2,
  },
  counterBtn: {
    width: BTN_SIZE, height: BTN_SIZE, borderRadius: BTN_SIZE / 2,
    borderWidth: 3, overflow: 'hidden', ...shadow.gold,
  },
  counterBtnGrad: {
  ...
  totalBadge: {
    alignItems: 'center', backgroundColor: colors.navy[900],
